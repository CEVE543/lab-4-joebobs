...
*How did your understanding of Austin's extreme rainfall evolve through the iterative prior specification process? What knowledge sources proved most valuable for extreme value analysis?

I got to know that the priors that we modelled for Austin were significant in analyzing the GEV. This is noted from the shift of the mean of the Posterior estimate to that of the MLE estimate. The most important knowledge source was likely the parameter priors (most likely shape) as the MLE curve seems to be less flat compared to the posterior fit curve.
*
...
I understand that the domain knowledge is a prior anyone has to account for while approching any problem. The ability to quantify that and use it in a weather simulation is an excellent way to prevent significant drifts in paramter fits especially in underdetermined systems. Bayesian analysis specifically is the interleave of both domain knowledge (priors) and empirical data (datapoints)*
...
I believe that under most conditions, it is fair to assume similar priors for stations that are nearby. Unless the climate of the two stations are affected by the geographical and climate factors, which can happen along fault lines or regions with extreme topographical phenomenon, they may use the same hand-crafter priors made from domain knowledge.
*
...
Bayesian uncertainity quantification is a more robust estimate of the climate phenomenon given the distribution that it is able to produce and the multitude of factors that it is able to consider like priors of parameters and return periods. I also noticed that while bayesian estimates gave more or less the same result each time I ran this notebook, the MLE approach was considerably heuristic and gave quite different results. The different results have been mentioned and displayed in an email I have sent to the Professor (also attached as a zip here). Further the Diagnostics results show the explainability of the bayesian approaches and provide strong future priors for the areas we have analyzed.*
...
As we have some intuition about the true GEV curves for Houston area stations (through domain knowledge), the iterative process helps us understand what priors are most important in modelling weather phenomenon in areas around Houston. For instance, if the MLE estimate gives us unusually high location or shape parameter, we will know what kind of priors we should make sure to ensure for Bayesian estimates. Further, considering the iterative workflow involving two nearby stations, we are able to use the bayesian approach paired with well defined priors we got from a better characterized nearby area to get a true estimate of its GEV curves. I also think that the iterative approach demonstrates the strength of domain knowledge in climate modelling. Challenges involved may be (as I am new to climate studies), devising appropriate priors for stations around houston.
*
...
### Loading Austin Precipitation Data
...
**Before looking at any data**, think about what you know about extreme rainfall in Austin.
What's your frame of reference?
Are you thinking about:

- General knowledge of rainfall patterns anywhere?
- Austin's specific climate characteristics?
- Personal experience with storms?
- Engineering design standards?

As a starting point, I wrote down some initial very weak priors. 
However, they are likely not very realistic for Austin's climate.
...
Are these return level curves reasonable for Austin's climate, based on your domain knowledge?
:::
...
```{julia}
comparison_stnid = 112 # MODIFY THIS AND CHOOSE YOUR COMPARISON
